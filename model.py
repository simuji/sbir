...
from Networks import VGG_Network, InceptionV3_Network,baseline,Resnet50_Network
...
class FGSBIR_Model(nn.Module):
    ...
    def train_model(self, batch):
        ...
        #print(F.pairwise_distance(sample_feature,positive_feature)-F.pairwise_distance(sample_feature,negative_feature))
...
class FGSBIR_Model1(nn.Module):
    def __init__(self, hp):
        super(FGSBIR_Model1, self).__init__()
        self.sample_embedding_network = eval('baseline(hp)')
        self.loss = nn.TripletMarginLoss(margin=0.2)
        self.sample_train_params = self.sample_embedding_network.parameters()
        self.optimizer = optim.Adam(self.sample_train_params, hp.learning_rate)
        self.hp = hp
    ...
    def train_model(self, batch):
        ...
        positive_feature,negative_feature,sample_feature = self.sample_embedding_network(batch['positive_img'].to(device),batch['negative_img'].to(device),batch['sketch_img'].to(device))
        #print(F.pairwise_distance(sample_feature,positive_feature)-F.pairwise_distance(sample_feature,negative_feature))
    ...
    def test_forward(self, batch):            #  this is being called only during evaluation
        positive_feature,sketch_feature = self.sample_embedding_network(batch['sketch_img'].to(device),batch['positive_img'].to(device),batch['positive_img'].to(device),False)
